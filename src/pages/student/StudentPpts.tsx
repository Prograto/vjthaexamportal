...
import { Presentation, Download, Loader2, Search, Eye } from "lucide-react";
...
export default function StudentPpts() {
  ...
  const handleDownload = async (m: any) => {
    ...
    try {
      ...
      const s3Url = res.data.data.s3Url;
      if (!s3Url) { toast.error("File URL not available"); return; }
      ...
      <Dialog open={!!viewing} onOpenChange={() => setViewing(null)}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
          ...
          <div className="flex-1 overflow-hidden rounded-xl border border-slate-100">
            {viewing?.s3Url && (
              <iframe src={getDocumentPreviewUrl(viewing.s3Url, 'ppt')} className="w-full h-full" title={viewing.title} />
            )}
          </div>
        </DialogContent>
      </Dialog>
